...
This feature is in Preview and the API may change in a future release.
...
Before we move on, let's add an index to the `ordersRollup` pre-aggregation so
...
With all the above set up, making a query such as the following will now use
